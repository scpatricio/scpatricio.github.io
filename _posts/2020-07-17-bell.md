...
This makes it clear that this distribution is capable of adapting to overdispersed data without the addition of a parameter, such as the Negative Binomial distribution. If this overdispersion was not clear, we can repair the distribution, taking $$ \mu = \theta e^\theta $$ and hence $$ \theta = W_0(\mu) $$, where$$ W_0(\mu) $$ in the Lambert function. Then
